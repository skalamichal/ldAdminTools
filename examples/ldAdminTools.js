...
angular.module('ldAdminTools')
	...
	.controller('ldSlideLeftController', ['$scope', '$transition', '$attrs', '$parse', '$log', 'ldSlideLeftConfig',
		function ($scope, $transition, $attrs, $parse, $log, config) {
			var self = this;
			//var scope = $scope.$new();
			...
			var isAnimated = angular.isDefined($attrs.ldAnimated) ? $scope.$eval($attrs.ldAnimated) : config.isAnimated;
			var isCloseable = angular.isDefined($attrs.ldCloseable) ? $scope.$eval($attrs.ldCloseable) : config.isCloseable;
			var isOpen = $scope.$eval($attrs.ldSlideLeft);
			...
			this.toggle = function (open) {
				isOpen = arguments.length ? !!open : !isOpen;
			...
			this.expand = function () {
				if (!isAnimated || initialAnimSkip) {
					initialAnimSkip = false;
					expandDone();
				}
				else {
					self.$element.removeClass('ld-slide');
					self.$element.addClass('ld-sliding-left');
					$log.debug('Expand - Left:');
					var a = doTransition({left: 0 + 'px'}).then(expandDone);
					$log.debug(a);
				}
			};
